      ...
      <div className="card-body p-4 sm:p-6">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-2 bg-gradient-to-br from-primary to-secondary rounded-xl shrink-0">
        ...
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1 sm:pr-2">
          {isLoading ? (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 p-3 sm:p-5">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                    <div className="relative size-11 sm:size-14 rounded-xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center shrink-0">
                      <Code2Icon className="size-5 sm:size-7 text-white" />
                      <div className="absolute -top-1 -right-1 size-3.5 sm:size-4 bg-success rounded-full border-2 border-base-100" />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-sm opacity-80">
                        <div className="flex items-center gap-1.5">
                          <CrownIcon className="size-3.5 sm:size-4 shrink-0" />
                          <span className="font-medium truncate max-w-[120px] sm:max-w-none">{session.host?.name}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <UsersIcon className="size-3.5 sm:size-4 shrink-0" />
                  ...
                  {/* ACTION BUTTON */}
                  <div className="flex justify-end sm:justify-normal shrink-0">
                    {session.participant && !isUserInSession(session) ? (
                      <button className="btn btn-disabled btn-sm w-full sm:w-auto">Full</button>
                    ) : (
                      <Link to={`/session/${session._id}`} className="btn btn-primary btn-sm gap-2 w-full sm:w-auto justify-center">
            ...
            <div className="text-center py-12 sm:py-16">
              <div className="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 bg-gradient-to-br from-primary/20 to-secondary/20 rounded-3xl flex items-center justify-center">
